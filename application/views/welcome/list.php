	...
	<table>
		...
		<%foreach $show_list as $row%>
			<tr>
				...
				<td><%if isset($row.spot_pic) and $row.spot_pic%><img src="<%if $row.from=="sjtu"%>/sjtu/piccontent?f=<%else%><%/if%><%$row.spot_pic%>" 
				style="width:100px;" /><%else%>暂无<%/if%></td>
				<td><a href="/welcome/viewinfo?from=<%$row.from%>&pid=<%$row.post_id%>">查看</a></td>
			</tr>
		<%/foreach%>
		</tbody>
	</table>
